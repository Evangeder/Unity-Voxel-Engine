            ...
            while (true)
            {
                ...
                for (int coord = 0; coord < _drawingArray.Length; coord++)
                {
                    newChunkPos = new int3(
                        _drawingArray[coord].x * BlockSettings.ChunkSize + ChunkLOD.PlayerPosition_int.x,
                        ChunkLOD.PlayerPosition_int.y,
                        _drawingArray[coord].y * BlockSettings.ChunkSize + ChunkLOD.PlayerPosition_int.z);
                    ...
                    for (int iy = -1; iy <= 1; iy++)
                        for (int iz = -1; iz <= 1; iz++)
                            if (ix != 0 && iy != 0 && iz != 0)
                            {
                                ...
                                bool doesUpdatedictContainThis = updateDict.TryGetValue(tempChPos, out _);
                                bool doesChunksToDeleteContainThis = chunksToDelete.Contains(tempChPos);
